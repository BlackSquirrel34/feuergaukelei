...
export default function Header() {
  return <header className="z-[999] relative">
    ...
    <nav className="fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex w-[22rem] flew-wrap items-center justify-center gap-y-1 text-[0.9rem]
        font-medium text-slate-500 sm:w-[initial] sm:flew-nowrap sm:gap-5">
            {
            links.map(link => (
                ...
                >
                    <Link className="flewx w-full items-center justify-center px-3 py-3
                     hover:text-slate-950 transition" href={link.hash}>{link.name}</Link>
                </motion.li>
            ))
            }
